...
export default function HaulageLogisticsScreen() {
  ...
  const [selectedHaulageType, setSelectedHaulageType] = useState('SMALL_VAN');
  ...
  const fetchLocationIQRoute = async (origin, destination) => {
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        return { coordinates, distance: distanceInKm };
      }
      throw new Error('No route found');
    } catch (error) {
      console.error('LocationIQ Error:', error);
      Alert.alert('Route Error', 'Could not calculate route. Try again.');
      return null;
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchCard}>
        ...
        <TouchableOpacity style={styles.locationRow} onPress={goToSearchDestination}>
          ...
          <Text style={[styles.addressText, !dropoffAddress && styles.placeholder]}>
            {dropoffAddress || 'Where to deliver?'}
          </Text>
          {dropoffAddress && (
            <TouchableOpacity
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Choose vehicle type</Text>
                ...
                >
                  <View style={styles.rideOptionLeft}>
                    ...
                    <View>
                      ...
                      <Text style={styles.rideOptionEta}>For moving goods</Text>
                    </View>
                  </View>
      ...
      {!dropoffLocation && (
        <View style={styles.instructionsTooltip}>
          ...
          <Text style={styles.instructionsText}>Tap "Where to deliver?" to start</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  addressText: { marginLeft: 12, fontSize: 17, color: '#000', flex: 1 },
  placeholder: { color: '#999', fontStyle: 'italic' },
  clearButton: { marginLeft: 8, padding: 4 },
  divider: { height: 1, backgroundColor: '#eee', marginVertical: 8 },
  routeInfo: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#eee' },
  ...
  requestText: { color: 'white', fontWeight: '700', fontSize: 18 },
  pickupMarker: {
    backgroundColor: '#00B0F3',
    padding: 12,
    borderRadius: 30,
  ...
  instructionsText: { marginLeft: 8, fontSize: 13, color: '#666', flex: 1 },
});