...
// ONLY CHANGE: Ride types replaced with Haulage types
const HAULAGE_TYPES = [
  { id: 'SMALL_VAN', name: 'Small Van', icon: 'car-outline', color: '#00B0F3', multiplier: 3.0, eta: '~15 min' },
  { id: 'PICKUP_TRUCK', name: 'Pickup Truck', icon: 'car-sport-outline', color: '#4ADE80', multiplier: 3.2, eta: '~20 min' },
  { id: '5TON_TRUCK', name: '5-Ton Truck', icon: 'bus-outline', color: '#F59E0B', multiplier: 3.8, eta: '~30 min' },
  { id: '10TON_TRUCK', name: '10-Ton Truck', icon: 'construct-outline', color: '#EF4444', multiplier: 4.5, eta: '~45 min' },
];
...
export default function HaulageLogisticsScreen() {
  ...
  const [selectedHaulageType, setSelectedHaulageType] = useState('SMALL_VAN'); // Changed initial
  const [estimatedFare, setEstimatedFare] = useState(null);
  const [routeCoords, setRouteCoords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showRideModal, setShowRideModal] = useState(false); // Same modal name
  const [routeDistance, setRouteDistance] = useState(0);
  const [routeDuration, setRouteDuration] = useState(0);

  // Trip tracking states (EXACTLY THE SAME)
  const [tripStatus, setTripStatus] = useState(null);
  const [driverData, setDriverData] = useState(null);
  const [driverLocation, setDriverLocation] = useState(null);

  // WebSocket state (EXACTLY THE SAME)
  const [wsReady, setWsReady] = useState(false);
  const [connectionAttempts, setConnectionAttempts] = useState(0);

  // Map ready flag (EXACTLY THE SAME)
  const [isMapReady, setIsMapReady] = useState(false);

  // ────────────────────────────────────────────────
  //  WEBSOCKET LIFECYCLE (EXACTLY THE SAME)
  // ────────────────────────────────────────────────
  ...
  // ────────────────────────────────────────────────
  //  TRIP EVENT LISTENERS (only when wsReady) - EXACTLY THE SAME
  // ────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const handleTripAccepted = (data) => {
      console.log('🚚 Driver accepted haulage trip:', data); // Changed log
      setDriverData(data);
    ...
    const handleTripStarted = () => {
      console.log('▶️ Haulage trip started'); // Changed log
      setTripStatus('trip_started');
      Alert.alert('Trip Started', 'Your haulage trip has begun!'); // Updated text
    };

    const handleTripCompleted = (data) => {
      console.log('✅ Haulage trip completed:', data); // Changed log
      setTripStatus('trip_completed');
      setDriverData(null);
      setDriverLocation(null);
      Alert.alert(
        'Trip Completed',
        `Haulage trip completed successfully! Fare: ₦${data?.fare || estimatedFare || '—'}`, // Updated text
        [
          {
            text: 'Rate Driver',
            onPress: () =>
    ...
    const handleTripCancelledByDriver = (data) => {
      console.log('❌ Driver cancelled haulage trip:', data); // Changed log
      setTripStatus(null);
      setDriverData(null);
      setDriverLocation(null);
      Alert.alert('Trip Cancelled', 'The driver has cancelled the haulage trip.'); // Updated text
    };

    const handleTripCancelledByPassenger = () => {
      console.log('❌ Customer cancelled haulage trip'); // Changed log
      setTripStatus(null);
      setDriverData(null);
      setDriverLocation(null);
    };
  ...
  // ────────────────────────────────────────────────
  //  LOCATION & MAP INITIALIZATION (EXACTLY THE SAME)
  // ────────────────────────────────────────────────
  ...
  // ────────────────────────────────────────────────
  //  GOOGLE SERVICES (EXACTLY THE SAME)
  // ────────────────────────────────────────────────
  ...
  const displayRoute = async (origin, destination) => {
    ...
    if (routeData) {
      ...
      const haulage = HAULAGE_TYPES.find((h) => h.id === selectedHaulageType); // Changed variable name
      const fare = Math.round(500 + routeData.distance * 150 * haulage.multiplier); // SAME FORMULA
  ...
  // ────────────────────────────────────────────────
  //  USER ACTIONS (EXACTLY THE SAME, just changed variable names)
  // ────────────────────────────────────────────────
  ...
  const selectHaulage = (haulageId) => { // Changed function name
    setSelectedHaulageType(haulageId);
    setShowRideModal(false);

    if (routeDistance > 0) {
      const haulage = HAULAGE_TYPES.find((h) => h.id === haulageId); // Changed variable name
      const fare = Math.round(500 + routeDistance * 150 * haulage.multiplier); // SAME FORMULA
      setEstimatedFare(fare);
    }
  };
  ...
  if (!currentLocation) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading map...</Text> {/* Same text */}
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
        ...
        {pickupLocation && (
          <Marker coordinate={pickupLocation}>
            <View style={styles.pickupMarker}>
              <Ionicons name="cube" size={30} color="white" /> {/* Changed icon */}
            </View>
          </Marker>
        )}
        ...
        {dropoffLocation && (
          <Marker coordinate={dropoffLocation}>
            <View style={styles.dropoffMarker}>
              <Ionicons name="business" size={30} color="white" /> {/* Changed icon */}
            </View>
          </Marker>
        )}
        ...
        {driverLocation && (
          <Marker coordinate={driverLocation} anchor={{ x: 0.5, y: 0.5 }}>
            <View style={styles.driverMarker}>
              <Ionicons name="truck" size={30} color="white" /> {/* Changed icon */}
            </View>
          </Marker>
        )}
      ...
      {/* Socket Status (debug only) - EXACTLY THE SAME */}
      {__DEV__ && (
      ...
      {/* Active Trip Overlay - EXACTLY THE SAME, just text changes */}
      {tripStatus && (
        <View style={styles.tripStatusCard}>
          <View style={styles.tripStatusHeader}>
            <Ionicons
              name={tripStatus === 'driver_nearby' ? 'truck' : 'checkmark-circle'} 
              size={24}
              color="#00B0F3"
            />
            <Text style={styles.tripStatusText}>
              {tripStatus === 'driver_nearby' ? 'Driver on the way' : 'Haulage in progress'} {/* Updated text */}
            </Text>
          </View>
          ...
          {driverData && (
            <>
              ...
              <TouchableOpacity style={styles.cancelTripButton} onPress={cancelTrip}>
                <Text style={styles.cancelTripText}>Cancel Haulage</Text> {/* Updated text */}
              </TouchableOpacity>
            </>
          )}
        </View>
      ...
      {/* Search Card - shown when no active trip - EXACTLY THE SAME, just placeholder text */}
      {!tripStatus && (
        <View style={styles.searchCard}>
          <View style={styles.locationRow}>
            <Ionicons name="cube" size={22} color="#00B0F3" /> {/* Changed icon */}
          ...
          <TouchableOpacity style={styles.locationRow} onPress={goToSearchDestination}>
            <Ionicons name="business" size={22} color="#FF4444" /> {/* Changed icon */}
            <Text style={[styles.addressText, !dropoffAddress && styles.placeholder]}>
              {dropoffAddress || 'Where to deliver?'} {/* Updated placeholder */}
            </Text>
      ...
      {/* Ride Type Modal - EXACTLY THE SAME, just data changed */}
      <Modal visible={showRideModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Choose a truck</Text> {/* Updated title */}
            ...
            <FlatList
              data={HAULAGE_TYPES} // Changed data
              keyExtractor={(item) => item.id}
              renderItem={({ item }) => (
                <TouchableOpacity
                  style={[
                    styles.rideOption,
                    selectedHaulageType === item.id && styles.selectedOption, // Changed variable
                  ]}
                  onPress={() => selectHaulage(item.id)} // Changed function
                >
                    ...
                    {estimatedFare
                      ? Math.round(
                          (estimatedFare * item.multiplier) /
                            HAULAGE_TYPES.find((h) => h.id === selectedHaulageType).multiplier // Changed variable
                        ).toLocaleString()
                      : '--'}
                  </Text>
                </TouchableOpacity>
      ...
      {/* Bottom Panel - shown when destination selected and no active trip - EXACTLY THE SAME, just text changes */}
        ...
        <View style={styles.bottomPanel}>
          <TouchableOpacity style={styles.rideSelector} onPress={() => setShowRideModal(true)}>
            <View style={styles.rideLeft}>
              <View
                style={[
                  styles.rideIcon,
                  { backgroundColor: HAULAGE_TYPES.find((h) => h.id === selectedHaulageType)?.color }, // Changed variable
                ]}
              >
                <Ionicons
                  name={HAULAGE_TYPES.find((h) => h.id === selectedHaulageType)?.icon} // Changed variable
                  size={28}
                  color="white"
                />
              </View>
              <View>
                <Text style={styles.rideName}>
                  {HAULAGE_TYPES.find((h) => h.id === selectedHaulageType)?.name} {/* Changed variable */}
                </Text>
                <Text style={styles.rideEta}>
                  {HAULAGE_TYPES.find((h) => h.id === selectedHaulageType)?.eta} {/* Changed variable */}
                </Text>
              </View>
            </View>
          ...
          <View style={styles.requestArea}>
            <View>
              ...
              <Text style={styles.distanceText}>Approximate haulage cost</Text> {/* Updated text */}
            </View>

            <TouchableOpacity
              style={styles.requestBtn}
              onPress={() => {
                navigation.navigate('DriverMatching', { // Changed screen name
                  pickup: pickupLocation,
                  dropoff: dropoffLocation,
                  pickupAddress,
                  dropoffAddress,
                  serviceType: selectedHaulageType, // Changed variable
                  estimatedFare,
            ...
            >
              <Text style={styles.requestText}>Request Truck</Text> {/* Updated text */}
            </TouchableOpacity>
          </View>
        </View>
      )}
...
// Styles remain EXACTLY THE SAME as PassengerHomeScreen
const styles = StyleSheet.create({
  ...
  // Markers (SAME, but you might want different icons)
  driverMarker: {
